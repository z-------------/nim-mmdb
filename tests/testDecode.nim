...
test "bytes":
  for _, (stringEncoded, expected) in strings:
    let
      encoded = (stringEncoded[0].uint8 xor 0xC0).char & stringEncoded[1..stringEncoded.high]
      decoded = decode(newStringStream(encoded))
    check decoded.kind == mdkBytes
    check decoded.bytesVal == expected
